...
| `BRAZIL_STAGE_TIMEOUT_S` | `30.0` | Per-stage ceiling (seconds) for one PCDT/BVS search stage in the Brazilian MoH engine. The BVS host holds a connection ~19-25 s before its first byte even when Solr answers in under half a second, so a lower value cancels responses that were about to arrive. |
