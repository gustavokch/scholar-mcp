...
| `BRAZIL_STAGE_TIMEOUT_S` | `30.0` | Per-stage ceiling (seconds) for one PCDT/BVS search stage in the Brazilian MoH engine. The BVS host holds a connection ~19-25 s before its first byte even when Solr answers in under half a second, so a lower value cancels responses that were about to arrive. Raising it past `SCHOLAR_REQUEST_TIMEOUT` (default `30`) buys nothing on its own: that is the ceiling on each individual HTTP attempt, so both must move together. |
| `BRAZIL_CHAIN_TIMEOUT_S` | `120.0` | Whole-chain ceiling (seconds) for `search_brazil_moh_guidelines`: bounds the PCDT stage, all BVS stages, and the camoufox browser tier together. Keep it above `3 x BRAZIL_STAGE_TIMEOUT_S` so full-ceiling stages cannot starve the browser tier. `<= 0` disables the bound. |
...
| `BRAZIL_FULLTEXT_TIMEOUT_S` | `30.0` | Whole-call ceiling (seconds) for `get_brazil_moh_full_text` network work (record lookup plus PDF fetch), separate from `BRAZIL_CHAIN_TIMEOUT_S`. One deadline covers both phases and bounds the 5xx retry ladder inside each: no share is reserved for the PDF fetch, because cutting the lookup short would lose the record and the abstract fallback with it. A budget exhausted after the lookup succeeded still returns the abstract, flagged `abstract_fallback`. |
